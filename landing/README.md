...
`node landing/prepare-media.mjs` rebuilds the cropped preview and poster from the existing source.

Builds fingerprint CSS, JS and media and expose `/site/version.json` with the committed source revision. Commit before deploying and verify the marker. If rollback is needed, deploy a previous version of `hiob-creative-landing`; the other route owners are independent.
...
The CSP allows the existing GTM, Meta Pixel, Cloudflare analytics and the exact two Meta-configured CAPI gateway hosts observed on the production pixel. It does not allow arbitrary cloud hosting domains. Browser privacy controls and third-party cookie blocking may still affect attribution.
