  ...
  return (
    <main className="bg-light min-h-screen">
      <nav style={{ padding: '1.05rem clamp(1rem, 4vw, 2rem)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '4px solid var(--border-dark)' }}>
        <a href="/" aria-label="hiob 홈" style={{ textDecoration: 'none', color: 'inherit' }}>
          <Logo height={28} />
        </a>
      ...
      <section style={{ padding: 'clamp(3rem, 8vw, 5rem) clamp(1.1rem, 4vw, 2rem)', borderBottom: '4px solid var(--border-dark)' }}>
        <div className="wrap">
          <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="massive-text" style={{ color: 'var(--hiob-orange)', wordBreak: 'keep-all' }}>{c.hero1}</motion.h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '2rem' }}>
            {c.points.map((p, i) => (
              <div key={i} className="brutalist-card">
                <h3 style={{ fontSize: 'clamp(1.4rem, 4.8vw, 2rem)', marginBottom: '1rem' }}>{p.t}</h3>
      ...
      <section style={{ padding: 'clamp(3rem, 8vw, 5rem) clamp(1.1rem, 4vw, 2rem)', background: '#000', color: '#fff', textAlign: 'center' }}>
        ...
        <a href="https://open.kakao.com/o/srdaF2si" target="_blank" rel="noopener noreferrer" className="btn-brutal primary" style={{ fontSize: 'clamp(1.12rem, 3.4vw, 1.5rem)' }}>
          {c.cta}
        </a>
      </section>
      <MobileStickyCTA label="컨설팅 문의하기" />
      <Footer lang={lang} />
    </main>
  );
}
